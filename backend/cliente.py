...
Versión con ZONA HORARIA BOLIVIA (UTC-4)
"""
...
import subprocess
import shutil
import pytz  # 👈 NUEVO - Zona horaria

# Zona horaria de Bolivia (UTC-4)
BOLIVIA_TZ = pytz.timezone('America/La_Paz')

def get_bolivia_time():
    """Retorna la hora actual en Bolivia como objeto datetime timezone-aware"""
    return datetime.now(BOLIVIA_TZ)

def get_bolivia_time_str():
    """Retorna la hora actual en Bolivia como string ISO"""
    return get_bolivia_time().isoformat()

def get_bolivia_time_formatted():
    """Retorna la hora actual en Bolivia formateada para mostrar"""
    return get_bolivia_time().strftime('%Y-%m-%d %H:%M:%S')
...
class ClusterCliente:
    def __init__(self, servidor_host=None, servidor_port=9999):
        ...
        print(f"{'='*60}")
        print(f"🕐 Hora Bolivia: {get_bolivia_time_formatted()}")
    ...
    def obtener_info_disco(self):
        ...
        INCLUYE TIMESTAMP CON HORA BOLIVIA
        """
        try:
            ...
            return {
                ...
                'timestamp': get_bolivia_time_str()  # 👈 HORA BOLIVIA
            }
    ...
    def conectar(self):
        ...
        try:
            ...
            # Enviar registro inicial con hora Bolivia
            self.enviar_mensaje({
                'tipo': 'REGISTRO',
    ...
    def enviar_metricas(self):
        """
        Envía métricas periódicamente con hora Bolivia
        """
        while self.running:
            try:
                ...
                if metricas:
                    ...
                    if self.enviar_mensaje(mensaje):
                        hora_bolivia = get_bolivia_time().strftime('%H:%M:%S')
                        print(f"📤 [{hora_bolivia}] Métricas: {metricas['capacidad_total']}GB, {metricas['porcentaje_uso']}% usado ({metricas['tipo_disco']})")
                    else:
    ...
    def procesar_mensaje(self, mensaje):
        ...
        if tipo == 'BIENVENIDA':
            ...
            # Enviar ACK (confirmación) con hora Bolivia
            ack_mensaje = {
                'tipo': 'ACK',
    ...
    def _guardar_log(self, mensaje):
        """Guarda en archivo de log con hora Bolivia"""
        try:
            with open('cliente.log', 'a', encoding='utf-8') as f:
                timestamp = get_bolivia_time_formatted()
    ...
    def iniciar(self):
        ...
        print(f"🕐 Hora Bolivia actual: {get_bolivia_time_formatted()}")
...
def main():
    ...
    try:
        ...
        for i, d in enumerate(discos):
            try:
                uso = shutil.disk_usage(d.mountpoint)
                print(f"   {i+1}. {d.device} - {round(uso.total/(1024**3),2)}GB")
            except:
                print(f"   {i+1}. {d.device} - No accesible")
    except:
        print("   No se pudo obtener información de discos")
